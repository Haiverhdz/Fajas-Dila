...
### Ronda 5 — enum real de `status` del webhook (6 valores) + columna `addi_status`

El usuario consiguió el enum completo y confirmado del campo `status` que manda Addi en el webhook: **`APPROVED`, `PENDING`, `REJECTED`, `ABANDONED`, `DECLINED`, `INTERNAL_ERROR`** (reemplaza el guess anterior de solo 4 valores en minúscula). Cambios:

- **Nueva columna `orders.addi_status`** — `ENUM('APPROVED','PENDING','REJECTED','ABANDONED','DECLINED','INTERNAL_ERROR') NULL`, el estado **crudo** tal como lo manda Addi, separada de `orders.status` (nuestro estado normalizado). NULL para pedidos de Wompi o antes del primer webhook.
- **`types/payment.ts`**: nuevo tipo `AddiRawStatus` (los 6 valores). `OrderStatus` (nuestro enum interno) quedó en `pending | approved | declined | in_process | abandoned | error` — se **quitó `'rejected'`** (una suposición de la Ronda 2, ya no aplica) y se **agregó `'error'`**.
- **Mapeo en `lib/addi.ts` (`mapAddiStatus`)**: `APPROVED→approved`, `PENDING→pending`, `REJECTED` y `DECLINED` se **fusionan** en `declined` (decisión: no hay hoy una razón operativa para mostrarle al cliente/admin algo distinto entre "Addi te rechazó el crédito" y "el pago se declinó" — ambos significan lo mismo para nosotros: no se puede despachar el pedido), `ABANDONED→abandoned`. **`INTERNAL_ERROR` es a propósito su propio estado (`'error'`)**, no se mapea a `declined`: no es que el cliente no haya calificado, es una falla técnica del lado de Addi que probablemente necesita seguimiento manual — así el admin puede distinguir ambos casos en la tabla de pedidos.
- **`lib/order.ts`**: `updateOrderStatus()` ahora acepta un tercer parámetro opcional `addiStatus` (el webhook de Wompi lo sigue llamando sin él, sin romper nada) y todas las consultas (`getOrdersByUserId`, `getAllOrders`, `getOrderByReference`) incluyen la columna nueva.
- **`lib/db/migrations/001_add_addi_statuses.sql` se corrigió en el mismo archivo** (nunca se había ejecutado, así que no hacía falta una migración 002 separada solo para arreglar la 001) — ahora deja `status` en el enum final de 6 valores y agrega `addi_status`. **Sigue sin ejecutarse** — pendiente en Workbench.
- **UI**: `StatusBadge`/`STATUS_LABELS`, el filtro de `/admin/pedidos` y `AddiRetornoStatus.tsx` (página `/pago/addi/retorno`) actualizados para el nuevo set de estados. El detalle del admin (`/admin/pedidos/[reference]`) ahora muestra el `addi_status` crudo cuando existe, útil para diagnosticar un pedido en `error`.
- **Dato de referencia para el futuro, sin cambios de código:** el usuario también mencionó el campo `callbackStatus` (`SENT | RECOVERABLE_ERROR | UNRECOVERABLE_ERROR | NOT_SENT | NOT_AVAILABLE`), que pertenece al schema `CallbackInformationResponse` — la respuesta de un endpoint de **consulta** (probablemente para ver si Addi logró entregar una notificación), **distinto de nuestro propio webhook**. No se implementó nada al respecto; anotado por si algún día se agrega ese endpoint de consulta.

**Próximo paso al retomar:** (1) llevarle a Addi el hallazgo de la Ronda 4 (401 genérico de `api.addi-staging.com` + 403 "Service not enabled" al pedir el audience alterno) para que confirmen si falta aprovisionar algo en su lado; (2) que el usuario pegue `OnlineLoanApplicationCallbackRequest`/`CallbackInformationResponse` del Swagger de integración; (3) correr `lib/db/migrations/001_add_addi_statuses.sql` en Workbench (`status` al enum de 6 valores + columna `addi_status` nueva — `ADDI_WEBHOOK_USER`/`PASSWORD` ya están en `.env.local`, con el fix de escapado de `$`/`#`). Fuera de esto, sigue pendiente de la ronda de metadata anterior: confirmar en Vercel que `NEXT_PUBLIC_SITE_URL` no tiene espacios y que el deployment activo se construyó después de cualquier cambio a esa variable, conseguir llaves reales de Wompi (sandbox) y probar el Widget Checkout end-to-end, y (fuera de roadmap) email de confirmación con Resend.
